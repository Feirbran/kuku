# game_manager.gd (Versione con Fix Match Verificato + Attivazione Bottoni)
extends Node3D
class_name GameManager

# Assegna la scena CardVisual.tscn qui nell'Inspector!
@export var card_scene: PackedScene
# Assegna i bottoni dall'UI qui nell'Inspector!
@export var swap_button: Button
@export var pass_button: Button
# @export var swap_deck_button: Button # Per futuro bottone mazziere
# aggiunti bottoni per condizione player = mazziere
@export var swap_to_deck_button: Button
@export var pass_as_dealer_button: Button
...
func _advance_turn():
	...
	if next_player_candidate != -1:
		...
		_update_player_action_buttons() # <-- Aggiorna stato bottoni per nuovo turno
...
func _go_to_dealer_phase():
	...
	_update_player_action_buttons() # <-- Aggiorna stato bottoni per fase mazziere
...
func _on_pass_turn_button_pressed():
	...
	print(">> Pass Button Pressed: State=%s, CurrentPlayer=%d, HasSwapped=%s" % [
		GameState.keys()[current_state], current_player_index,
	...
	# Azione Umano: Passa (tiene la carta)
	...
	# Azione Mazziere Umano: Non scambia con mazzo
		...
		print("   -> Azione bottone Passa non valida.")
...
func _on_swap_button_pressed():
	...
	else: print("   -> Azione bottone Scambia non valida.")

func _on_card_clicked(card_visual: CardVisual):
	# Scambio al click disabilitato, usare i bottoni
	print("Click su carta ignorato (usare bottoni Scambia/Passa).")
...
func _player_action(player_index: int, action: String, target_player_index: int = -1):
	...
	if action == "swap":
		...
		if target_player_index < 0 or target_player_index >= players_data.size() or players_data[target_player_index].is_out or target_player_index == player_index:
			...
			# --- LOGICA RE/CAVALLO VA QUI ---
			# elif my_card.rank_name == "K": print("Hai Re, no swap"); return # Non fa nulla
			# elif target_card.rank_name == "K": print("Target ha Re, no swap"); return # Non fa nulla
			else:
				# Esegui lo scambio
	...
	if performed_action:
		if player_index == 0: _update_player_action_buttons() # Disabilita bottoni dopo azione umana
...
func _dealer_action(action: String):
	...
	_update_player_action_buttons() # Disabilita bottoni giocatore dopo azione mazziere
...
func _make_cpu_turn():
	...
	var target_player_index = get_player_to_left(cpu_player_index) # CPU scambia a sinistra per ora
...
func _end_round():
	if current_state == GameState.GAME_OVER: return
	_update_player_action_buttons() # Assicura bottoni disabilitati
...
func determine_loser_and_update_lives():
	...
	for i in range(players_data.size()):
		if not players_data[i].is_out:
			...
			else: printerr("  ERRORE: Impossibile leggere carta Player %d!" % i)
	print("--- Calcolo Perdente ---")
	for i in range(players_data.size()):
		if not players_data[i].is_out:
			var card_to_evaluate: CardData = _get_valid_carddata_from_player(i, "det_loser_calc")
...
func get_player_to_right(player_index: int) -> int:
	var current = player_index; var size = players_data.size(); if size <= 1: return -1
	for _i in range(size): current = (current + 1) % size; if current == player_index: return -1; if not players_data[current].is_out: return current
	return -1
...
func _get_valid_carddata_from_player(player_index: int, context: String = "?") -> CardData:
	...
	else: printerr("ERRORE (%s): Tipo non valido in card_data[%d]!" % [context, 0]); return null
...
func _update_player_action_buttons():
	# Validazione dei riferimenti ai bottoni
	...
	# Determina stati di abilitazione/visibilità
	...
	if current_state == GameState.PLAYER_TURN and \
	   ...
	   not players_data[0].has_swapped_this_round:
		...
		# print(">>> UI Update: Player Actions Enabled") # Debug opzionale
	...
	if current_state == GameState.DEALER_SWAP and \
	   ...
	   not players_data[0].is_cpu:
		...
		# print(">>> UI Update: Dealer Actions Enabled") # Debug opzionale

	# Aggiorna bottoni normali (Swap a destra, Pass normale)
	if normal_swap_valid:
		swap_button.disabled = not enable_player_buttons
		swap_button.visible = enable_player_buttons # Mostra solo nel turno normale
	if normal_pass_valid:
		pass_button.disabled = not enable_player_buttons
		pass_button.visible = enable_player_buttons # Mostra solo nel turno normale

	# Aggiorna bottoni specifici del mazziere
	if dealer_swap_valid:
		swap_to_deck_button.disabled = not enable_dealer_buttons
		swap_to_deck_button.visible = enable_dealer_buttons # Mostra solo nella fase mazziere
	if dealer_pass_valid:
		pass_as_dealer_button.disabled = not enable_dealer_buttons
		pass_as_dealer_button.visible = enable_dealer_buttons # Mostra solo nella fase mazziere

	# Assicurati che i set di bottoni non siano visibili contemporaneamente
	# (Questo è già gestito dalla logica 'visible = enable_...',
	# dato che enable_player_buttons e enable_dealer_buttons sono mutuamente esclusivi)
	
#endregion

# Funzione chiamata quando il bottone "Scambia" viene premuto
# NOTA: Questa era duplicata alla fine del file precedente, assicurati ci sia solo UNA definizione
#func _on_swap_button_pressed():
...
# Aggiungi queste funzioni in game_manager.gd

func _on_swap_to_deck_pressed():
	# Stampa stato per debug
	print(">> SwapDeck Button Pressed: State=%s, CurrentPlayer=%d" % [GameState.keys()[current_state], current_player_index])
	# Verifica se siamo nello stato e giocatore corretto (Mazziere umano)
	if current_state == GameState.DEALER_SWAP and current_player_index == 0 and not players_data[0].is_cpu:
		...
		# I bottoni verranno disabilitati/nascosti dalla chiamata a _update_player_action_buttons
		# che avviene dentro _dealer_action -> _end_round -> _start_round
	else:
...
func _on_pass_as_dealer_pressed():
	# Stampa stato per debug
	print(">> PassDealer Button Pressed: State=%s, CurrentPlayer=%d" % [GameState.keys()[current_state], current_player_index])
	# Verifica se siamo nello stato e giocatore corretto (Mazziere umano)
	if current_state == GameState.DEALER_SWAP and current_player_index == 0 and not players_data[0].is_cpu:
